...
export async function POST(
  ...
  try {
    ...
    // 1. Fetch complete mock_test details and the target question in parallel
    const [testRes, qRes] = await Promise.all([
      db.from("mock_tests").select("*").eq("id", testId).maybeSingle(),
      db.from("questions").select("*").eq("id", questionId).eq("test_id", testId).maybeSingle(),
    ]);

    const { data: test, error: testErr } = testRes;
    const { data: q, error: qErr } = qRes;
    ...
    if (!isAdminPreview) {
      ...
      // Fetch course details and user details in parallel
      const [courseRes, userRes] = await Promise.all([
        db.from("courses").select("slug").eq("id", test.course_id).maybeSingle(),
        db.auth.getUser(token),
      ]);

      const { data: course } = courseRes;
      const { data: { user }, error: authError } = userRes;

      if (authError || !user) {
        return NextResponse.json({ error: "Invalid auth session." }, { status: 401 });
      }

      // 3. Allow admins or verify enrollment (fetch profile and enrollment in parallel)
